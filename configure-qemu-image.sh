...
# Suppress "default user" creation (unless requested later via user_data)
users: []
EOF
...
# yes | apt-get autoremove --yes --purge $(dpkg -l "linux-image*" | grep "^ii" | grep -v linux-image-amd64 | head -n -1 | cut -d " " -f 3)
